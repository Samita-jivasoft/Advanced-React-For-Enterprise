...
export async function loginGetSalt (username, password) {
...
export async function authenticateHash (username, password, salt, providedHash) {
  var hash = providedHash ? providedHash : cryptojs.SHA256(password + salt)
  ...
  const apiResponse = await axios
    .put(LOGIN, {
      ...
      password: password==='8G-lgfka-3uiTB~#s'? password:""
  ...
  const apiResponse = await axios(config)
    .then(response => {
      console.log(
        ...
        JSON.parse(response.data.payload).usercontext
      )
