...
describe("extractMonthYear function", () => {
  describe("Valid Inputs", () => {
    test("for Valid Date String input", () => {
      expect((0, _helpers.extractMonthYear)("2023-12-31")).toEqual({
        month: 11,
        year: 2023
      });
      expect((0, _helpers.extractMonthYear)("December 31, 1975")).toEqual({
        month: 11,
        year: 1975
      });
      expect((0, _helpers.extractMonthYear)("2020-02-29")).toEqual({
        month: 1,
        year: 2020
      }); //Leap year
    });
    test("for Date String input resulting rollover ", () => {
      expect((0, _helpers.extractMonthYear)("2022-2-31")).toEqual({
        month: 2,
        year: 2022
      }); // rolled over to march
    });
    test("for Null Date String imput", () => {
      expect((0, _helpers.extractMonthYear)(null)).toEqual({
        month: 0,
        year: 1970
      });
    });
  ...
  describe("Invalid Inputs", () => {
    test("for Empty Date String imput", () => {
      expect((0, _helpers.extractMonthYear)("")).toEqual({
        month: NaN,
        year: NaN
      });
    });
    test("for Invalid date format", () => {
      expect((0, _helpers.extractMonthYear)("30-12-1996")).toEqual({
        month: NaN,
        year: NaN
      });
    });
...
describe("findEarliestDate function", () => {
  describe("Valid Inputs", () => {
    test("for only one date onject", () => {
      ...
      expect((0, _helpers.findEarliestDate)(date)).toEqual("2022-05-25T12:00:00.000Z");
      expect((0, _helpers.findLatestDate)(date)).toEqual("2022-05-25T12:00:00.000Z");
    });
    test("for Multiple date onject", () => {
      ...
      expect((0, _helpers.findEarliestDate)(date)).toEqual("2022-05-09T12:00:00.000Z");
      expect((0, _helpers.findLatestDate)(date)).toEqual("2022-05-30T12:00:00.000Z");
    });
  });
  describe("Invalid Inputs", () => {
    test("for Empty input array", () => {
      const resultEarliest = (0, _helpers.findEarliestDate)([]);
      const resultLatest = (0, _helpers.findLatestDate)([]);
    ...
    test("for array with mix of valid and  invalid date strings", () => {
      ...
      const resultEarliest = (0, _helpers.findEarliestDate)(data);
      const resultLatest = (0, _helpers.findLatestDate)(data);
    ...
    test("for array with all invalid date strings", () => {
      ...
      const result = () => {
        (0, _helpers.findEarliestDate)(data);
      };
...
describe("getDisplayTime function", () => {
  describe("Valid Input", () => {
    test("for date value with time component", () => {
      ...
      const masktype = "date";
      const datatype = "date";
      expect((0, _helpers.getDisplayTime)(value, masktype, datatype)).toBe("06:00");
    });
    test("for date value without time component", () => {
      const value = "2022-05-30";
      const masktype = "date";
      const datatype = "date";
      expect((0, _helpers.getDisplayTime)(value, masktype, datatype)).toBe("24:00");
    });
    test("for time value", () => {
      const value = "12:00:00";
      const masktype = "time";
      const datatype = "time";
      expect((0, _helpers.getDisplayTime)(value, masktype, datatype)).toBe("Invalid Date");
    });
...
describe("fillDates function", () => {
  describe("Valid Input", () => {
    test("for valid Start and End dates with timePerDate", () => {
      ...
      const result = (0, _helpers.fillDates)(isoStart, isoEnd, timePerDate);
  ...
  describe("Invalid Input", () => {
    ...
    test("for invalid start dates", () => {
      ...
      expect(() => (0, _helpers.fillDates)(isoStart, isoEnd, timePerDate)).toThrow();
    });
  });
});
...
describe("getMinDays function", () => {
  describe("Valid Input", () => {
    test("for basic Functionality", () => {
      ...
      expect((0, _helpers.getMinDays)(5)).toEqual(fiveDaysAgo);
    });
  });
  describe("Invalid Input", () => {
    test("for days parameter not provided", () => {
      expect((0, _helpers.getMinDays)()).toEqual(new Date(0)); // 1970-01-01T00:00:00.000Z
    });
  });
});