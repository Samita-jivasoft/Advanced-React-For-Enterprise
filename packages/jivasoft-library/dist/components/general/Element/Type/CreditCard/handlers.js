...
const enforceCustomFormat = (value, formatOrFormats, cursorPosition) => {
  ...
  sortedFormats.forEach(format => {
    ...
    for (let i = 0; i < format.length; i++) {
      ...
      if (formatChar === "#") {
        ...
        if ((formattedValue[valueIndex - 1] !== formatChar || formattedValue[valueIndex] !== formatChar) && valueIndex < value.length) {
  ...
  if (cursorPosition > 0 && cursorPosition <= bestFit.formattedValue.length && !Array.from(bestFit.formattedValue).filter(char => char !== "#").includes(bestFit.formattedValue[cursorPosition - 1])) {
    cursorPosition++;
  }
  return {
