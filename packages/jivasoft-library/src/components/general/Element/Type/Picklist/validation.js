...
export function getPickerValidation (elementState) {
  const {
    value,
    validminimum,
    validmaximum,
...
function validatePickRange (
  ...
  if (allowmultiplepicklistselections == 1) {
    if (
      (validminimum === undefined && validmaximum === undefined) ||
      ...
      validminimum === undefined &&
      validmaximum &&
      value?.length < validminimum
    ) {
      return (
        'This number of selections must be between ' +
        validminimum +
        ' and ' +
        validmaximum
      )
    } else if (value?.length < validminimum) {
      return (
        'This value must contain at least ' + validminimum + ' selection(s)'
      )
    } else if (value?.length > validmaximum) {
      return 'Cannot contain greater than ' + validmaximum + ' selection(s)'
    } else {
      return false
    }
