export function ISO8601ToDayMonthYear (date) {
...
export function generateDefaultValue (value, datatype) {
  switch (datatype) {
    case 'datetime':
      return value
    case 'time':
...
export function handleDateChangeOriginal (
  value,
  dateValue,
  setDateValue,
  masktype
...
export function handleDateChange (
  ...
  if (!masktype || masktype === 'datetime') {
    ...
    // console.log('time', time)
    newDateValue.setHours(time?.split(':')[0], time.split(':')[1], 0)
    // if (
    //   ((time?.split(':')[0] == '24' || time?.split(':')[0] == '00') &&
    //     time?.split(':')[0] == '24') ||
    //   time?.split(':')[0] == '00'
...
export function handleTimePickerParent (
  value,
  duration,
  required,
  elementDispatch,
