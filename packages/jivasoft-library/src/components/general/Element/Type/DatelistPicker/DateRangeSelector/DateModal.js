...
import { returnDate } from '../../../../../../app/helpers'
import {
  RowTr,
  RowTd,
  ThDiv,
...
export const DateRange = ({
  ...
  useEffect(() => {
    ...
    timeout = setTimeout(() => {
      if (dateVal.startdate) {
        if (
          dateVal.startdate > tempArr[tempArr.length - 1].date.slice(0, 10) ||
      ...
      if (dateVal.enddate) {
        if (
          dateVal.enddate < dateVal.startdate ||
          dateVal.enddate < tempArr[0].date.slice(0, 10)
        ) {
          setDateWarning({ dateWarning, startWarning: true, endWarning: true })
        } else {
          setDateWarning({
      ...
      if (
        dtArray.length !== 0 &&
        dateVal.enddate &&
        dateVal.enddate >= tempArr[0].date &&
        dateVal.enddate != tempArr[tempArr.length - 1].date
      ) {
        ...
        if (dateStr < endDateStr && endDateStr < endLimitDate.toISOString()) {
          ...
          var dateIndex = tempArrCopy.findIndex(
            element => element.date === dateVal.enddate
          )
          ...
          for (var i = 0; i < dateArrCopy.length; i++) {
            if (
              dateArrCopy[i].date.slice(0, 10) <=
              tempArrCopy[dateIndex].date.slice(0, 10)
            ) {
              dateArrIndex = i
            }
          }
      ...
      if (
        dtArray.length > 0 &&
        dateVal.startdate &&
        dateVal.startdate <= tempArr[tempArrLength - 1].date &&
        dateVal.startdate != tempArr[0].date
      ) {
        ...
        if (
          dateVal.startdate < tempArr[0].date &&
          dateVal.startdate > startLimitDate.toISOString()
        ) {
          ...
          dateVal.startdate > tempArr[0].date &&
          dateVal.startdate <= tempArr[tempArr.length - 1].date
        ) {
          ...
          var dateIndex = tempArrCopy.findIndex(
            element => element.date === dateVal.startdate
          )
          var dateArrIndex = dateArrCopy.findIndex(
            element => element.date >= tempArrCopy[dateIndex].date
          )
  ...
  useEffect(() => {
    ...
    timeout = setTimeout(() => {
      ...
      if (
        timeVal.starttime &&
        timeVal.endtime &&
        timeVal.starttime > timeVal.endtime
      ) {
        setTimeWarning(true)
      } else {
  ...
  useEffect(() => {
    if (dateArr.length > 0) {
      if (
        (tempArr[0].date < dateArr[0].date.slice(0, 10) &&
          tempArr[tempArr.length - 1].date < dateArr[0].date.slice(0, 10)) ||
        (tempArr[0].date > dateArr[dateArr.length - 1].date.slice(0, 10) &&
          tempArr[tempArr.length - 1].date >
            dateArr[dateArr.length - 1].date.slice(0, 10))
      ) {
        var objArr = []
        var modObjArr = []
  ...
  function handleSelecting (idx, masktype) {
    ...
    if (tempArrItem.selected === true) {
      ...
      while (
        index < dateArrCopy.length &&
        dateArrCopy[index].date.slice(0, 10) < tempArrItem.date
      ) {
        index++
      }
      ...
      while (
        index < dateArrCopy.length &&
        dateArrCopy[index].date.slice(0, 10) < dateArrayItem.date.slice(0, 10)
      ) {
        index++
      }
      ...
      for (let i = 0; i < tempArrCopy.length; i++) {
        if (tempArrCopy[i].day === dayNum) {
          ...
          if (isFound) {
            ...
            while (
              index < dateArrCopy.length &&
              dateArrCopy[index].date.slice(0, 10) < tempArrCopy[i].date
            ) {
              index++
            }
          ...
          <div style={{ display: 'flex', flexDirection: 'row' }}>
            <div>
              ...
              <DateInput
                ...
                onChange={e =>
                  setDateVal({ ...dateVal, startdate: e.target.value })
                }
              />
              <NameDiv>
            ...
            <div>
              ...
              <DateInput
                ...
                onChange={e =>
                  setDateVal({ ...dateVal, enddate: e.target.value })
                }
              />
              <NameDiv>
                ...
                <TimePicker
                  ...
                  handleParent={value => {
                    if (value) {
                      ...
                      setTimeVal({ ...timeVal, starttime: time[0] })
                    }
                  }}
                  canedit={1}
                />
        ...
        headerText={
          tempArr.length > 0 && (
            <div style={{ marginLeft: 'calc(10px + 5vmin)' }}>
              {returnDate(tempArr[0].date.slice(0, 10))} -{' '}
              {returnDate(tempArr[tempArr.length - 1].date.slice(0, 10))}
            </div>
          )
        }
      >
