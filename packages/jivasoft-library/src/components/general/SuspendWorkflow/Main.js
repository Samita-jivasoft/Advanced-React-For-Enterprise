...
import { useStep, useSuspend, useAppTheme} from 'app/data'
...
import { AnimatedDynamicModal } from 'components'
...
export const SuspendedWorkflow =()=>{
        ...
        appDispatch({
          ...
          currentModal:(
            ...
            >
              <ModalDiv>
                <ModalButton
                  onClick={()=>{PopulateDefaultValue(selectedSuspendState,selectedStepState,selectedStepData,selectedStepStateIndex);appDispatch({type:'UNSET_MODAL'})}}
                >
                  Yes
                </ModalButton>
  ...
  function PopulateDefaultValue(selectedSuspendState,selectedStepState,selectedStepData,selectedStepStateIndex){
      ...
      if(selectedStepState.steptype === 'form')
      {
        ...
        setTimeout(()=>{
          appDispatch({type:'SELECT_AGENCY',currentAgency:selectedSuspendState.suspendStateAgency})
          appDispatch({type:'SELECT_WORKFLOW',currentWorkflow:selectedSuspendState.suspendStateWorkflow,isSuspended:true}) 
          generateStepContext(stepDispatch,selectedStepState.stepstate)
          suspendDispatch({type:'REMOVE_STATE',payload:selectedStepStateIndex})
        },'0')
      }
      else{
        return;
  ...
  return(
    <OverlayDiv>
      ...
      <FooterDiv>
        ...
        <WorkflowButton
          onClick={()=>{suspendDispatch({type:'DELETE_ALL'}); setShowImgModal(false); setShowWorkflowModal(true)}}
        >
          Delete All Workflow
        </WorkflowButton>
      </FooterDiv>
