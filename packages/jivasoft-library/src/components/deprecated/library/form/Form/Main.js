...
export const Form = (props) => {
  ...
  return (
    <>
      ...
      >
        <FormStyledHeader>
          {tabState && <Tabs selected={selected} setSelected={setSelected} groups={tabState} />}
        ...
        {renderedForm}
        {(tabState && (tabState?.indexOf(selected) !== tabState?.length - 1 && tabState?.length > 1)) && <DynamicButtonV2
          text={tabState[tabState.indexOf(selected) + 1]?.groupid ? tabState[tabState.indexOf(selected) + 1]?.groupid?.toUpperCase() : 'Next'}
          onClick={() => {
